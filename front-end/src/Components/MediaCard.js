...
export default function MediaCard(props) {
	...
	const [data, isLoading] = useFetch(
		`${process.env.HOST_API_URL}?getUserFile&file_ID=${mediaFile.ID}&thumb`,
		"FILE"
	)
	...
	async function handleClickOnDownload() {
		...
		const actionURL = `${process.env.HOST_API_URL}?getUserFile&file_ID=${mediaFile.ID}`;
	...
	async function handleClickOnDelete() {
		const actionURL = `${process.env.HOST_API_URL}?deleteUserFile&file_ID=${mediaFile.ID}`;
		const response = await fetch(actionURL, {
			method: "DELETE"
		});
